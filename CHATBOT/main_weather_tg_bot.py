...
# Замените 'tg_bot_token' на ваш токен, полученный от BotFather в Telegram
...
def get_weather(city, open_weather_token, user_action, chat_id):
    if user_action == 1:
        try:
    ...
    if user_action == 2:
        try:
            ...
            def create_dict_with_temp(data):
                days = []
                hours = []
                dict_days_and_hours = {}
                list_to_append = []
            ...
            plot = plt.plot(dates, temperatures, marker='o', linestyle='-')
    ...
    '''
    if user_action == 3:
        bot.send_message(chat_id, "Напишите город")
...
def handle_monitoring_city(message: Message):
    chat_id = message.chat.id
    city = message.text
...
def handle_threshold(message: Message, city: str):
    chat_id = message.chat.id
    try:
...
def handle_monitoring_direction(call):
...
def handle_user_action(message: Message):
...
def handle_city(message: Message, user_action: int):
    city = message.text
